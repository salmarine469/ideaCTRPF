...
namespace CTRPluginFramework
{
  ...
  u16 Convert::sjisToUtf16(const u16 &sjis)
  {
    ...
    {
      int mid = (low + high) / 2;
      u16 guess = sjis_utf16List[mid].SJIS;
      if (guess == sjis)
      {
        return sjis_utf16List[mid].UTF16;
      }
      else if (guess > sjis)
      {
        high = mid - 1;
  ...
  u16 Convert::utf16ToSjis(const u16 &utf16)
  {
    ...
    {
      int mid = (low + high) / 2;
      u16 guess = utf16_sjisList[mid].UTF16;
      if (guess == utf16)
      {
        return utf16_sjisList[mid].SJIS;
      }
      else if (guess > utf16)
      {
        high = mid - 1;
  ...
  void Convert::addHiraganaKanjiList(const std::string &hiragana)
  {
    char *res = NULL;
    u32 size = 0;
    std::string out = "", url = "http://www.google.com/transliterate?langpair=ja-Hira|ja&text=" + hiragana + ',';
    std::vector<u8> buff;
    if (Curl::Get(url.c_str(), buff) != CURLE_OK)
      return;
    int count = 0;
    for (auto &&_ : buff)
    {
